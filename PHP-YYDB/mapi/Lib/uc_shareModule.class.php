    ...
    */
    public function index(){
        ...
        if($user_login_status!=LOGIN_STATUS_LOGINED){
			...
			$luck_list = $GLOBALS['db']->getAll("select id,duobao_item_id,name,deal_icon,is_send_share from ".DB_PREFIX."deal_order_item where user_id = ".$user_info['id']." and delivery_status=1 and is_arrival=1 and is_send_share=0 and type=0 order by id desc");
			$share_list = $GLOBALS['db']->getAll("select a.id,a.duobao_item_id,a.name,a.deal_icon,a.is_send_share,b.is_check,b.id as share_id,b.user_id,b.user_name,b.create_time,b.is_effect,b.title,b.content,b.image_list from ".DB_PREFIX."deal_order_item as a left join ".DB_PREFIX."share as b on a.duobao_item_id = b.duobao_item_id where a.user_id = ".$user_info['id']." and a.is_send_share=1 and a.type=0 order by a.id desc"." limit " . $limit);
			$share_list_count = $GLOBALS['db']->getOne("select count(a.id) from ".DB_PREFIX."deal_order_item as a left join ".DB_PREFIX."share as b on a.duobao_item_id = b.duobao_item_id where a.user_id = ".$user_info['id']." and a.is_send_share=1 and a.type=0");
