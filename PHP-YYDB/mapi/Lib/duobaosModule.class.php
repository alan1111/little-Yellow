<?php
class duobaosApiModule extends MainBaseApiModule{
    /**
     * 全部商品列表接口
     * 输入：
     * data_id: int 分页ID
    ...
    public function index(){
        ...
        $sql = "SELECT id,unit_price, name, max_buy, min_buy,current_buy, (max_buy-current_buy) as surplus_buy, icon
                    FROM
                    	".DB_PREFIX."duobao_item
    		        WHERE
    		            is_effect = 1 AND success_time = 0 "; 
        if($cate_item)
        {
