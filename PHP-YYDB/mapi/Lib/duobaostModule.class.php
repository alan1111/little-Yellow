<?php
class duobaostApiModule extends MainBaseApiModule{
    /**
     * xx元全部商品列表接口
     * 输入：
     * data_id: int 分页ID
    ...
    public function index(){
        ...
        $sql_count = "SELECT count(*)
    		        ...
    		        WHERE
    		            is_effect = 1 AND
    		            success_time = 0 and min_buy = ".$min_buy." ";
        if($cate_item)
        {
        ...
        $sql = "SELECT id, name, max_buy,min_buy, current_buy, (max_buy-current_buy) as surplus_buy, icon
                    FROM
                    	".DB_PREFIX."duobao_item
    		        WHERE
    		            is_effect = 1 AND success_time = 0 and min_buy = ".$min_buy." "; 
        if($cate_item)
        {
