...
define("ACCOUNT_CREATE_ERROR",4); //帐户自动注册失败
define("ACCOUNTGC_NO_EXIST_ERROR",5); //帐户不存在
...
define("LOGIN_STATUS_TEMP",2); //临时登录
	...
	function auto_create($user_data,$type,$allow_err=false,$user_logo="")
	{
		...
		if($user_id)
		{
			...
			if($user_data['pid']!=0){
			    if(app_conf("USER_INVITE_SCORE")>0){
			...
			if($GLOBALS['ref_uid']>0){
				...
				modify_account($user_log,intval($GLOBALS['ref_uid']),"在".to_date(NOW_TIME)."注册成功邀请返积分");
			}
			...
			if($mode == 'INSERT')
			{
				...
				if($user_data['pid']!=0){
					if(app_conf("USER_INVITE_SCORE")>0){
						...
						modify_account($data,$user_data['pid'],$log_msg);
					}
				}
...
/**
		 ...
		 * $allow_err : 是否报错
		 */
		function ms_auto_create($user_data,$type,$allow_err=false,$user_logo="")
		{
			if($type<0||$type>2)$type=0;
	                if($type==1){
		                    $user_data['is_phone_register'] = 1;
		...
		function ms_do_login_user($user_name_or_email,$user_pwd)
		{
			//通过数据库实现
	                         /*define('BASE_PATH',str_replace('system/model/', '', str_replace('\\','/',realpath(dirname(__FILE__).'/'))."/"));

	                         require_once BASE_PATH."./system/db/SQLHelper.php";

	                         $SQLHelper = new SQLHelper();
	                         $sql ="select * from  Accounts_Users where userid=4";
	                        $user_data = $SQLHelper->getRow($sql);*/

	                       /* $url='http://www.nthxmai.com/ida/account/dologin?u='.$user_name_or_email.'&p='.$user_pwd; */
	                        /*$url='http://localhost:8059/ida/account/dologin?u='.$user_name_or_email.'&p='.$user_pwd; 

	                        $ResultInfo=json_decode(file_get_contents($url), true) ;*/

	                         $ResultInfo= get_remotedata('ida/account/dologin','u='.$user_name_or_email.'&p='.$user_pwd,1);

			if($ResultInfo["status"])
			{
	                                   /* $user_data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where suid='".$ResultInfo["DATA"]["userID"]);
*/
	                             /*   $userid=$ResultInfo["DATA"]["userID"];
	                                $user_data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where suid=".$userid."  and is_delete = 0  ");*/
	                            ...
	                            /*    $user_data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where suid =".$ResultInfo["DATA"]["userID"]);*/
	                                    ...
	                                    {
		                                    	$user_data = array();
		                                    	$user_data['suid'] = $ResultInfo["data"]["userID"];
		                                    	$user_data['user_name'] = $ResultInfo["data"]["phone"];
		                                    	$user_data['mobile'] = $ResultInfo["data"]["phone"];
		                                    	/*$user_data['user_pwd'] = $ResultInfo["data"]["password"];*/

		                                     if(isset($user_pwd)&&$user_pwd!=''){
		                                    	        $user_data['user_pwd'] = md5($user_pwd);
		                                      }

		                                    	if(isset($ResultInfo["data"]["trueName"]))
		                                    	{
		                                    	      $user_data['real_name'] = $ResultInfo["data"]["trueName"];	
		                                    	}
		                                    	else{
		                                    		$user_data['real_name'] = $ResultInfo["data"]["nickName"];
		                                    	}
		                                    	
		                                    	$rs_data = ms_auto_create($user_data, 1);
		                                    	if(!$rs_data['status'])
				             {
					        $result['status'] = 0;
					        $result['data'] = ACCOUNT_CREATE_ERROR;
					         return $result;
				             }
	                                    }
	...
	{
		$loginresult= ms_do_login_user($user_name_or_email,$user_pwd);
		...
		else
		{
			...
			else
			{
				...
				//后期实现跨域单点登录 zhu
				es_session::set("user_name", $user_name_or_email);
				es_session::set("user_pwd", $user_pwd);
