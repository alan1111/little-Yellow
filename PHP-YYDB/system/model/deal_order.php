		...
		if($order_info)
		{
			...
			if($GLOBALS['db']->affected_rows())
			{
				...
				if($order_info['pay_status']==0&&$order_info['pay_amount']>0)
				{
					...
					{
						modify_account(array("money"=>($order_info['pay_amount']-$order_info['ecv_money'])), $order_info['user_id'],"取消订单，退回余额支付 ");
						//增加退款到会员中心的充值记录
						modify_statements(($order_info['pay_amount']-$order_info['ecv_money']), 2, "交易取消，金额存入会员帐户");
						
						//收入
						modify_statements(($order_info['pay_amount']-$order_info['ecv_money']), 0,"交易取消，金额存入会员帐户"); //总收入
