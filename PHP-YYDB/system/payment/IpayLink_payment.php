	...
	public function get_redirect_url($payment_notice_id)
	{
        ...
        if ($transid) {
           ...
           if($from=="wap"){
               ...
               if($order_info['type']==1)
               {
                   $trade->redirecturl = SITE_DOMAIN.APP_ROOT.wap_url("index","user_center#index");
               }
               else
               {
                 //$trade->redirecturl = SITE_DOMAIN.APP_ROOT.wap_url("index","uc_duobao_record#index");
                   $trade->redirecturl = SITE_DOMAIN.APP_ROOT.wap_url("index","payment#done",array("id"=>$payment_notice['order_id']));
               }
           }else{
               // 调用pc端支付
               $trade->h5OrPcUrl = $trade->pcurl;
               $trade->redirecturl = SITE_DOMAIN.APP_ROOT.url("index","uc_duobao#index");
           }
