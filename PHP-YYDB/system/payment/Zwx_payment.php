...
class Zwx_payment implements payment {
    public function __construct(){
        $_GET['pay_class_name'] = 'Zwx';
    }
	...
	public function get_payment_code($payment_notice_id)
	{
		...
		$pay['pay_action'] = SITE_DOMAIN.APP_ROOT."/cgi/payment/Zwx/redirect.php?notice_id=".$payment_notice_id."&from=".$GLOBALS['request']['from'];
	...
	public function get_redirect_url($payment_notice_id)
	{
		...
		$money = $payment_notice['money'] * 100;
  		$money = round($money);
 		$notify_url = SITE_DOMAIN.APP_ROOT.'/callback/payment/Zwx_notify.php';
 		...
 		if (isMobile() ||  $from == 'wap' || IS_MOBILE == 1) {
 		    ...
 		    if (isWeixin()) {
 		        ...
 		        $trade_type = 'trade.weixin.wappay';
 		    }
 		    ...
 		    $trade_type = 'trade.weixin.native';
 		}
	...
	public function get_display_code(){
	    ...
	    if(isMobile() || IS_MOBILE == 1){
	        ...
	        $payment_item = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."payment where class_name='Zwx'");
	        if($payment_item)
	        {
