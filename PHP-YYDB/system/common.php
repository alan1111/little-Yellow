...
//$classname  array() 方法
//$param 参数
//$loginflg 是否登录方法 0 否 1 是 默认为否  注意 这里不能用true或者false不识别，切记切记
function get_remotedata($classname='',$param=null,$loginflg=0)
{
	//zhu edit 20170721 远程验证 begin
	
	$reurl='http://www.nthxmai.com/';
	/*$reurl='http://localhost:8059/';*/
	...
	$user_pwd  =es_session::get("user_pwd");
             ...
             if($classname==null || strlen($classname)<=0){
             	$result['status'] = 0;
		$result['data'] = '没有传入访问方法!';
		return $result;
             }

	if(loginflg==0)
	{
	             if($user_name==null || strlen($user_name)<=0||$user_pwd==null || strlen($user_pwd)<=0)
	             {
				...
				$remoteurl=$reurl.$classname.'?'.$param;
			}
	             }
	             else{
		             $remoteurl=$reurl.$classname.'?u='.$user_name.'&p='.$user_pwd;
		             if($param!=null&&strlen($param)>0){
	             	 	$remoteurl=$remoteurl.'&'.$param;
	            		 }
            		}
             }
	else{
		$remoteurl=$reurl.$classname;
	             if($param!=null&&strlen($param)>0){
             	 	$remoteurl=$remoteurl.'?'.$param;
            		 }
	}         

            $ResultInfo=json_decode(file_get_contents($remoteurl), true) ;

            if($ResultInfo["STATUS"]!="OK")
	{
		$result['status'] = 0;
		$result['data'] =$ResultInfo["DATA"];
		return $result;
	}
	else
	{
