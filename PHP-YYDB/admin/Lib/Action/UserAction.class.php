...
class UserAction extends CommonAction{
	...
	public function modify_batch_add(){
        ...
        $icon_star = M("Conf")->where(" name ="." 'ROBOT_LOGO_SORT' ")->getField("value");//头像开始下标
        ...
        for($i=2; $i<=$highestRow; $i++){
            ...
            $data['login_ip']= $objPHPExcel->getActiveSheet()->getCell("B".$i)->getValue();
...
//		$condition['is_robot'] = 0;
	...
	protected function _group_list($model, $map, $sortBy = '', $asc = false,$count=-1) {
	    ...
	    if ($count > 0) {
	        ...
	        //分页查询数据
	        $voList = $model->where($map)->order( "`" . $order . "` " . $sort)->limit($p->firstRow . ',' . $p->listRows)->group('rel_user_id')->field("*,sum(score) as sum_score")->select();
