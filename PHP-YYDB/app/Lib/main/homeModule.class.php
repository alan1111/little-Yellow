...
class homeModule extends MainBaseModule
{
	public function index()
	{
		...
		$sql_total = "select count(distinct(duobao_item_id)) from ".DB_PREFIX."deal_order_item where type = 2 and pay_status = 2 and user_id = ".$id;
		...
		$sql = "select *,sum(number) as number from ".DB_PREFIX."deal_order_item where type = 2 and pay_status = 2 and user_id = ".$id." group by duobao_item_id order by create_time desc limit ".$limit;
