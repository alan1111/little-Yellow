...
class duobaoModule extends MainBaseModule
{
    ...
    {//倒计时刷新
        ...
        $root['duobao_recode_count'] = $GLOBALS['db']->getOne($total_sql);
        $root['duobao_recode_list']=$list;
         if($duobao_status==2)
        {
            $duobao_item_log = $GLOBALS['db']->getRow("select * from ".duobao_item_log_table($item_data)." where lottery_sn = '".$item_data['lottery_sn']."' and duobao_item_id = ".$item_data['id']);
            ...
            $duobao_item_log['user_duobao_count']=intval($GLOBALS['db']->getOne("select sum(doi.number) from ".DB_PREFIX."deal_order_item as doi left join ".DB_PREFIX."deal_order as do on doi.order_id = do.id where do.user_id = ".$duobao_item_log['user_id']." and doi.duobao_item_id = ".$item_data['id']." and doi.refund_status = 0 and do.pay_status = 2 and do.type = 2"));
            $duobao_item_log['province_name'] =$GLOBALS['db']->getOne("select name from ".DB_PREFIX."delivery_region where id = ".$user_info['province_id']);
            $duobao_item_log['city_name'] =$GLOBALS['db']->getOne("select name from ".DB_PREFIX."delivery_region where id = ".$user_info['city_id']);
            $item_data['luck_lottery'] = $duobao_item_log;
            ...
            if(!$GLOBALS['user_info']){ //没登录的情况下，查看他的号码，即中奖人的号码
                $ta_list=duobao::get_user_no_all(array("user_id"=>$user_info['id'],"duobao_item"=>$item_data));
                $ta_total_sql = "select sum(doi.number) from ".DB_PREFIX."deal_order_item as doi left join ".DB_PREFIX."deal_order as do on doi.order_id = do.id where do.user_id = ".$user_info['id']." and doi.duobao_item_id = ".$item_data['id']." and doi.refund_status = 0 and do.pay_status = 2 and do.type = 2 ";
                $root['is_login']=0;
            }else{
                if($GLOBALS['user_info']['id']!=$user_info['id']){//用户非中奖者，查看中奖者号码
                    $ta_list=duobao::get_user_no_all(array("user_id"=>$user_info['id'],"duobao_item"=>$item_data));
            ...
            $root['ta_duobao_recode_count'] = $GLOBALS['db']->getOne($ta_total_sql);
            $root['ta_duobao_recode_list']=$ta_list;
        }
        ...
        $root['item_data'] = $item_data;
        //最新一期
        $new_item_data= $GLOBALS['db']->getRow("select * from ".DB_PREFIX."duobao_item where duobao_id=".$item_data['duobao_id']." and id > ".$item_data['progress']." order by id desc limit 0,1");
        ...
        if($root['is_login']==0){
        	...
        	if($duobao_item_log_cache===false)
        	{
        		$ta_list=duobao::get_user_no_all(array("user_id"=>$user_info['id'],"duobao_item"=>$item_data));
        		$ta_total_sql = "select sum(doi.number) from ".DB_PREFIX."deal_order_item as doi left join ".DB_PREFIX."deal_order as do on doi.order_id = do.id where do.user_id = ".$user_info['id']." and doi.duobao_item_id = ".$item_data['id']." and doi.refund_status = 0 and do.pay_status = 2 and do.type = 2 ";
        		$duobao_recode_count=$GLOBALS['db']->getOne($ta_total_sql);
        		$duobao_item_log_arr=array();
        		$duobao_item_log_arr['duobao_list']=$ta_list;
        		$duobao_item_log_arr['duobao_recode_count']=$duobao_recode_count;
        		set_dynamic_cache("duobao_item_log_".$item_data['id'], $duobao_item_log_arr);
