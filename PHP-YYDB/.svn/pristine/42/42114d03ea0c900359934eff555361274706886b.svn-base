...
class userApiModule extends MainBaseApiModule
{
	...
	public function dologin()
	{
		...
		else
		{
			...
			/*return output("",0,$result['status'].','.$result['data']);*/
		if($result['status'])
		{
		...
		else
		{
			...
			else if($result['data'] == ACCOUNT_CREATE_ERROR)
			{
				$field = "user_key";
				$err = "用户自动注册失败";
			}
			else if($result['data'] == ACCOUNTGC_NO_EXIST_ERROR)
			{
				$field = "user_key";
				$err = "谷创商城夺宝用户不存在";
			}
			else if($result['data'] == ACCOUNT_ABNOR_ERROR)
			{
				$field = "user_key";
				$err = "该用户账号异常，请联系谷创客服!";
			}
        ...
        public function dophregister()
	{
		...
		if($res['status'] == 1)
		{
			...
			//在此自动登录
			do_login_user($user_data['mobile'],$user_data['user_pwd']);
