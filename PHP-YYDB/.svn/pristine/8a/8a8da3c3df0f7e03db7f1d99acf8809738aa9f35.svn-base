 ...
 * 获取分享列表
 */
function get_topic_list($limit, $param=array("cid"=>0,"tag"=>""),$join='', $where='',$orderby = '',$tname='',$append_field='')
{
	if(empty($param))
	...
	if($join)
		$sql = "select ".$tname.".*".$append_field." from ".DB_PREFIX."topic as ".$tname." ".$join." where  ".$condition;
	else
		$sql = "select ".$tname.".*".$append_field." from ".DB_PREFIX."topic as ".$tname." where  ".$condition;
...
function insert_topic($content, $title='', $type='', $group='', $relay_id = 0 , $fav_id = 0, $group_data = "", $attach_list = array() , $url_route=array(), $tags=array(), $xpoint="", $ypoint="",$forum_title='',$group_id=0,$syn_weibo )
{
	...
	if( $xpoint==''&&$ypoint=='' ){	//获取当前用户的lbs信息
		$current_geo = es_session::get("current_geo");
		$xpoint = $current_geo['xpoint'];
		$ypoint = $current_geo['ypoint'];
	}
...
function insert_topic_reply($reply_data){
	...
	if($id>0)
	{
		...
		if($reply_data['topic_user_id']!=$GLOBALS['user_info']['id']){
			send_msg($reply_data['topic_user_id'], "您的主题".$reply_data['topic_title']."被".$reply_data['user_name']."点评了", "topic", $reply_data['topic_id']);
		}
		if($reply_data['reply_id']>0){
			if($reply_data['reply_user_id']!=$GLOBALS['user_info']['id']){
				send_msg($reply_data['reply_user_id'], "您在主题".$reply_data['topic_title']."的点评被".$reply_data['user_name']."回复了", "topic", $reply_data['topic_id']);
			}			
		}
	}
...
/**
 * 获取发布的分享格式数据
 * @param unknown $ctl 模块
 * @param unknown $data_id
 * @return string
 */
function get_format_share_data($ctl,$data_id){
        ...
        switch($ctl)
        {
            case "deal":  //团购分享
                ...
                {
                    $result['status'] = 1;
                    $result['content'] = "我购买了：".$deal['sub_name'];
            ...
            case "youhui":
                ...
                {
                    $result['status'] = 1;
                    $result['content'] = "我领取了:".$youhui['name'];
            ...
            case "event":
                ...
                {
                    $result['status'] = 1;
                    $result['content'] = "我报名了:".$event['name'];
...
function order_share($order_id){
    global_run();
    require_once APP_ROOT_PATH."system/model/deal.php";
    require_once APP_ROOT_PATH."system/libs/words.php";

    $order_item = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_order_item where order_id=".$order_id);
    ...
    if($order_item){
        foreach($order_item as $k=>$v){
            ...
            if($deal)
            {
                ...
                $url_route = array();
                $url_route = array(
                    'rel_app_index'=>$group_data['url']['app_index'],
                    'rel_route'=>$group_data['url']['route'],
                    'rel_param'=>''
                );
                ...
                foreach($deal_gallery as $row)
                {
                    $images[] = syn_image_to_topic($row['img']);
                }
                ...
                foreach($images as $row)
                {
                    ...
                    $topic_image['id'] =  intval($row['id']);
                    $attach_list[] = $topic_image;
                }
            }

            $id = insert_topic($content,$title,$type,$group, $relay_id = 0, $fav_id = 0,$group_data,$attach_list,$url_route,$tags,'','','',0,0);
            if($id)
            {
