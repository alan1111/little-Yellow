...
// +----------------------------------------------------------------------
// | Author: zhuxiaohui（594177881@qq.com）
// +----------------------------------------------------------------------
...
class user_centerApiModule extends MainBaseApiModule
{
	...
	public function index()
	{
		...
		{
                                $root ['points_amount'] =$ResultInfo['data'];
		}
		else{
		       $root ['points_amount'] ='通宝获取失败';
		}
		...
		if($user_login_status!=LOGIN_STATUS_LOGINED){
			...
			// 时时查询用户是否开启渠道功能
			$root['is_open_scan'] = $GLOBALS['db']->getOne("select is_open_scan from ".DB_PREFIX."user where id={$user_id}");
