...
class shareApiModule extends MainBaseApiModule{
    ...
    public function index(){
    	...
    	$share = new share();
    	$result_list = $share->get_share_list($limit,$excondition='is_effect');
    	...
    	$sql_count="select count(*) from ".DB_PREFIX."share where is_effect=1";
    ...
    public function detail(){
        ...
        if(!$share_info){
            $sql="select * from ".DB_PREFIX."share where is_effect=0 and user_id=".$user_info['id']." and id=".$id;
            $share_info=$GLOBALS['db']->getRow($sql);
        }
    	if(!$share_info){
