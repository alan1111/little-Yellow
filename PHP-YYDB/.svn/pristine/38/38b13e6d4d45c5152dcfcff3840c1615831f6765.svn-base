...
{
    $user_id = intval($user_id);
    $inte_sql="SELECT u.user_name, SUM(r.money) AS money, SUM(r.score) AS score FROM ".DB_PREFIX."user AS u LEFT JOIN ".DB_PREFIX."referrals AS r ON u.id = r.rel_user_id WHERE u.pid = ".$user_id.".AND u.pid = r.user_id GROUP BY r.rel_user_id limit ".$limit;
