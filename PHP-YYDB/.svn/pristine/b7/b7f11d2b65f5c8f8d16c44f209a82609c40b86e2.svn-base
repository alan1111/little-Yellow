...
class duobaosModule extends MainBaseModule
{
    ...
    {   
        ...
        $sql = "SELECT di.id, di.name, di.max_buy,di.min_buy, di.unit_price,di.current_buy,di.click_count, di.progress, (di.max_buy-di.current_buy) as surplus_buy, di.icon 
                FROM ".DB_PREFIX."duobao_item as di 
                WHERE di.is_effect = 1 AND di.success_time = 0 AND di.progress < 100 ";
