...
class indexModule extends MainBaseModule
{
	...
	{		
		...
		{
			$sql_recomend = "SELECT
	                	DuobaoItem.id AS id, DuobaoItem.deal_id AS deal_id, DuobaoItem.duobao_id AS duobao_id, DuobaoItem.name AS duobaoitem_name,DuobaoItem.brief AS duobaoitem_brief,
			            DuobaoItem.icon AS icon, DuobaoItem.lottery_sn AS lottery_sn, DuobaoItem.has_lottery AS has_lottery, DuobaoItem.success_time AS success_time,
		...
		//获得活动，按分类分组前4个
		$duobao_item=$GLOBALS['db']->getAll("select *,(max_buy-current_buy) as less_buy from ".DB_PREFIX."duobao_item where is_effect=1 and success_time = 0 and progress < 100 order by cate_id,create_time desc");
        ...
        //最新上架
        $newest_sql="select * from ".DB_PREFIX."duobao_item where is_effect=1 and success_time = 0 and progress < 100 order by create_time desc limit 20";
