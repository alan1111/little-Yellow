...
// 		$GLOBALS['user_info'] = load_user(230);
        ...
        $sql = "select *,sum(number) as number from ".DB_PREFIX."deal_order_item where type = 2 and pay_status = 2 and user_id = ".$id." ".$log_type_condition." ".$log_time_type_condition." group by duobao_item_id order by create_time desc limit ".$limit;
