    ...
    {   //输出100元专区
      ...
      //  $GLOBALS['tmpl']->assign("wrap_type","1"); //首页宽屏展示
        ...
        $sql_count = "SELECT count(*)
                    ...
                    WHERE
                        is_effect = 1 AND
                        success_time = 0 and min_buy = ".$min_buy." and unit_price = 100 ";
        if($cate_item)
        {
        ...
        $sql = "SELECT id, name,unit_price, max_buy,min_buy, current_buy, progress, (max_buy-current_buy) as surplus_buy, icon
                FROM ".DB_PREFIX."duobao_item
                WHERE is_effect = 1 AND success_time = 0 and min_buy = ".$min_buy." and unit_price = 100 ";
        if($cate_item)
        {
