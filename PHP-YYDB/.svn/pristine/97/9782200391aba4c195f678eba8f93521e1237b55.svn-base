...
class uc_centerModule extends MainBaseModule
{
	public function index()
	{
		...
		$sql = "select *,sum(number) as number from ".DB_PREFIX."deal_order_item where type = 2 and pay_status = 2 and user_id = ".$id." group by duobao_item_id order by create_time desc limit 5";
