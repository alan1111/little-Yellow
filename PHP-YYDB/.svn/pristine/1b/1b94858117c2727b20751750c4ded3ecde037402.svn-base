...
class SAASAPIClient
{
     ...
     *
     * @param $url 服务地址
     * @param $args 具体的接口调用参数（除appid, timestamp, signature外），PHP数组对象，如：array("domain"=>"xxx.yydb.nthxmai.com")
    ...
    /**
     * 生成谷创商城系统间信息安全传递地址，将要传递的参数加密后附加到指定地址后面。
    ...
    /**
     * 加密指定的数组成生成谷创商城系统间传递的安全参数字符串，以便通过HTTP的GET请求或POST请求进行传递
     * @param $params 待附加的参数数组
     * @param $timeoutMinutes 安全参数过期时间（单位：分钟），小于等于0表示永不过期
     * @return 加密后的安全参数字符串
     */
