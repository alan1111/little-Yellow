		...
		if($is_weixin&&$deny_weixin==0)
		{
			...
			if(WEIXIN_TYPE=="platform")
			{
				...
				}else
				{
					...
					$url = 'http://service.www.nthxmai.com/weixin/create_url';
					$wx_url = $client->makeSecurityUrl($url, $params, $widthAppid, $timeoutMinutes);
					//var_dump($wx_url);exit;
					//$wx_url = 'http://service.www.nthxmai.com/weixin/create_url?from='.urlencode($back_url);
					app_redirect($wx_url);
				}
...
function init_app_page()
{
    ...
    if ($user_id > 0) {
        ...
        if ($oi['id'] > 0) {
            ...
            // 如果是app，设置分享参数
            $share_data['share_content'] = wap_url("index", "index");
            $share_data['share_imageUrl'] = $logo_image;
            $share_data['share_url'] = wap_url("index", "index");
            $share_data['share_key'] = '';
            $share_data['share_title'] = $site_name;
            $share_data = json_encode($share_data);
//             share_content:内容
//             share_imageUrl:图片url
//             share_url:点击跳转url
//             share_key:key用于服务器记录分享,可不传
//             share_title:标题
	...
	$GLOBALS['tmpl']->assign("pc_url",url("index","index",array("is_pc"=>1)));
        ...
        $GLOBALS['tmpl']->assign("m_config",getMConfig());
