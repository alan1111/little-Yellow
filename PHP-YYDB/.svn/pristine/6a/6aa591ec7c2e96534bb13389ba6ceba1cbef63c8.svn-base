...
class DuobaoOrderAction extends CommonAction{
	...
	public function view_order()
	{
		...
		$order_info = M("DealOrder")->where("id=".$id." and type = 2")->find();
		if(!$order_info)
		{
...
// 		$map['is_robot'] = 0;
	...
	{
		$id = intval($_REQUEST['id']);
		$order_info = M("DealOrderHistory")->where("id=".$id." and type = 2")->find();
		if(!$order_info)
		{
			$this->error(l("INVALID_ORDER"));
		}
