...
class ajaxModule extends MainBaseModule
{
	...
	public function del_cart()
	{
		...
		if(isset($_REQUEST['id']))
		{
			...
			$cart_item = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_cart where session_id ='".es_session::id()."' and id = ".$id);
		...
		if($cart_item)
		{
			$GLOBALS['db']->query("delete from ".DB_PREFIX."deal_cart  where session_id = '".es_session::id()."' and pid = '".$cart_item['deal_id']."'");
		}
	...
	public function count_buy_total(){
	    ...
	    $result = count_buy_total($payment,$account_money,$all_account_money,0,$all_account_points,$paypassword,$ecvsn,$ecvpassword,$goods_list,$paid_account_money,0,0,$bank_id);
